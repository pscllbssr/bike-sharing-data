...
Our first analysis focused on the influence of different variables on the trip duration.
...
model_lm <- lm(duration ~ h_distance + temp + prcp + dwpt + poly(temp, degree=2) + poly(prcp, degree=2) + poly(dwpt, degree=2)+ weekend, data = data)
...
Another approach to predict the duration of the trips using the same variable as before was to use a Generalise Additive Model (GAM).
...
The graph of these variables seem to have a linear relationship that remains flat throughout the graph, therefore, an effect is difficult to spot here.
...
The smooth term for the distance has the largest edf value `r #round(summary(model_gam)$s.table["s(Distance)", "edf"], 3)` and precipitation the lowest with a value of `r #round(summary(model_gam)$s.table["s(Precipitation)", "edf"], 3)`. The intercept is existing.

We made similar observations with the linear model used above. However, the GAM performs better: the R^2 is of `r #round(summary(model_gam)$r.squared,2)`.

## Estimating the trip duration with a Neural Network (Pascal)

We also want to see if we can get a better estimation of the trip duration with a neural net. In order to do this, we first scaled our dataset and splitted in a test- and train-part.
...
As a baseline model, we took a very simple net consisting of one predictor (the haversine distance) and one hidden layer containing a single neuron.
...
We wanted to see if we could achieve better results with different parameters. We added all available predictors and used the caret-package to try out different options of the neuronal net.
