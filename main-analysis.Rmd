...
- `start_station_latitude`, `start_station_longitude`: DECIMAL, DECIMAL, location of start station
- `end_station_id`: INTEGER, identifier of end station
- `end_station_latitude`, `end_station_longitude`: DECIMAL, DECIMAL, location of end station
- `city`: STRING, name of city
- `temp`: DECIMAL, approximate temperature during trip
...
```
```{r}
#same as above but with weather data
short_trips_02 <- nrow(subset(data_02, duration <= 120 & start_station_id == end_station_id))
short_trips_02

long_trips_02 <- nrow(subset(data_02, duration >= 86400))
long_trips_02

data_02 <- data_02[!(data_02$duration <= 120 & data_02$start_station_id == data_02$end_station_id) & !(data_02$duration >= 86400), ]
max(data_02$duration)
```
...
# Count the number of rows in the original dataframe
before_date_filter_02 <- nrow(data_02)
before_date_filter_02

# Filter the trips for the months between April and November
data_filtered_02 <- data_02 %>%
  filter(start_month >= 4 & start_month <= 11)

# Count the number of rows in the filtered dataframe
after_date_filter_02 <- nrow(data_filtered)
after_date_filter_02

# Calculate the number of removed trips
row_removed_02 <- before_date_filter - after_date_filter
row_removed_02
```
...
#SAME as above but with weather data
# Add a column for the day of the week
data_02$weekday <- weekdays(as.Date(data$started_at))
# Add a column
data_02$weekend <- ifelse(weekdays(as.Date(data$started_at)) %in% c("Saturday", "Sunday"), "Weekend", "Weekday")
```
...
To narrow our analysis furthermore, we will only consider the data coming from the top `top` stations in each city.
...
```
We removed `removed_top20` trips from our dataset, which then contained `r after_top20` trips.

```{r}
#SAME as above but with weather data
# Group the data by city and station ID, and calculate the number of trips
trips_by_station_02 <- data_02 %>%
  group_by(city, start_station_id) %>%
  summarize(num_trips = n())
...
top_stations_02 <- trips_by_station_02 %>%
  arrange(city, desc(num_trips)) %>%
  group_by(city) %>%
  top_n(top)

# Filter the original dataset to keep only trips from the top stations
data_top_stations_02 <- data_02 %>%
  semi_join(top_stations_02, by = c("city", "start_station_id"))

before_top20 <- nrow(data_02)
after_top20 <- nrow(data_top_stations_02)
removed_top20 <- before_top20 - after_top20

data_02 <- data_top_stations_02
head(data_02)
...
```{r}
#SAME as above but with weather data
data_mod_02 <- data_02 %>% mutate(start = as.ITime(started_at)) %>%
  mutate(end = as.ITime(ended_at))
head(data_mod_02)
```
...
```{r}
#Same as above but with weather data
data_filtered_02 <- data_02 %>% filter(!is.na(temp))

# Calculate the number of removed trips
removed_temp_02 <- nrow(data_02) - nrow(data_filtered_02)
removed_temp_02

data_02 <- data_filtered_02
...
```{r}
#SAME as above but with weather data
data_02$prcp <- ifelse(is.na(data_02$prcp), 0, data_02$prcp)
str(data_02$prcp)
```
...
pairs(trips ~ avg_temp + avg_prcp + month + weekday, data = station_220_data)
```
...
weather <- data_02 %>% select(c(city,duration, temp, dwpt, prcp, snow, wpgt,wdir,wspd))
...
The GAM model states that temperature, dew_point, wind direction and wind spead have a significant effect on the duration of a bike trip. Whereas, the precipitation doesn't seem to have significant impact.
