...
# predict 100m
predict_100 <- compute(trips_net, as.data.frame(scale(data.frame(h_distance = c(1000)), center = min['h_distance'], scale = max['h_distance'] - min['h_distance'])))

predict_100 <- predict_100$net.result * (max['duration'] - min['duration']) + min['duration']

```

According to the model a biker needs roughly `r round(predict_100)` seconds or ~`r round(predict_100 / 60)` minutes for 1km airline distance, including time for setting up and parking the bike at the end.
