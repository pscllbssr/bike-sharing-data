...
This model fits our data quite well. The R^2 is of `r round(summary(model_lm)$r.squared,2)`.
...
#retrieve only data about the duration and weather parameters 
weather <- data %>% select(c(duration, temp, prcp, price, weekend))
colnames(weather) <- c("Duration", "Temperature", "Precipitation", "Price", "Weekend")

#remove NA values and chose relevant weather parameters
weather<- na.omit(weather[, c("Duration", "Temperature", "Precipitation", "Price", "Weekend")])
...
template.graph.weather + aes(x = Temperature)
template.graph.weather + aes(x = Precipitation)
template.graph.weather + aes(x = Price)
template.graph.weather + aes(x = Weekend)
```

```{r gam2}
gam <- gam(log(Duration) ~ s(Temperature) + s(Precipitation) + s(Price) + Weekend,
              data = weather)
summary(gam)
...
The GAM model states that the temperature, the precipitations, the price and the distinction weekday/weekend have a significant effect on the duration of a bike trip. We made the same observations with a linear model. However, the linear model performed better.
