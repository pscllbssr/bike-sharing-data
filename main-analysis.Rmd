...
However, this model does not fit our data very well. The R^2 is of `r round(summary(model_lm)$r.squared,2)` and RMSE is `r round(sqrt(mean(model_lm$residuals^2)), 2)`.
...
```{r gam_dataset, message=FALSE, cache=TRUE}
...
```{r gam}
gam_model <- gam(log(Duration) ~ s(Temperature) + s(Dew_Point) + s(Precipitation), data = weather)
summary(gam_model)

plot(gam_model, residuals = TRUE, pages = 1, shade = TRUE)
```

The GAM model states that temperature, dew_point and precipitation have a significant effect on the duration of a bike trip. The smooth term temperature has the largest edf value `r #round(summary(gam_model)$s.table["s(Temperature)", "edf"], 3)` and precipitation the lowest with a value of `r #round(summary(gam_model)$s.table["s(Precipitation)", "edf"], 3)`. The intercept is existing.
...
```{r gam2, cache=TRUE}
model_gam <- gam(log(Duration) ~ s(Temperature) + s(Precipitation) + s(Distance) + Weekend,
              data = weather)
summary(model_gam)

plot(model_gam, residuals = TRUE, pages = 1, shade = TRUE)
```
...
When we model this over all six stations
...
We also want to see if we can get a better estimation of the trip duration with a neural net. In order to do this we first scale our dataset and split in a test- and train-part.
...
As a baseline model we take a very simple net consisting of one predictor (the haversine distance) and one hidden layer containing a single neuron.
...
The baseline already performs quite well, considering the obvious outliers of sightseeing trips:
...
The RMSE calculated with the data scaled back lies at `r round(rmse_rescaled, 3)`. This is already better than the linear model above, although we have to consider that the training data consists only of a subset and has filtered out round trips in this case.

Now we want to see if we can achieve better results with different parameters. For this we add all available predictors and use the caret-package to try out different options of the neuronal net.
...
The predictions of the tuned model look similar to the predictions of the baseline model at first sight.
...
By adding more predictors, layers and neurons the RMSE decreased only a little bit, from `r round(rmse_rescaled, 3)` to `r round(rmse_tuned_rescaled, 3)`. This shows that the haversine distance still is the main predictor for the duration of a trip. And means we might as well use the simple baseline model.
